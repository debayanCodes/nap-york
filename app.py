...
try:
    ...
    with st.sidebar:
        ...
        selected_languages = st.multiselect("Select Languages", all_languages, default=['All'])
        star_range = st.slider("Stars Count Range",
        ...
        if 'created_at' in data.columns:
            date_range = st.date_input("Date Range", value=(data['created_at'].min(), data['created_at'].max()))
    ...
    if star_range:
        filtered_data = filtered_data[(filtered_data['stars_count'] >= star_range[0]) & 
                                      (filtered_data['stars_count'] <= star_range[1])]
    if 'created_at' in data.columns and date_range:
        filtered_data = filtered_data[(filtered_data['created_at'] >= pd.to_datetime(date_range[0])) & 
    ...
    with tab1:
        ...
        with col1:
            ...
            fig = px.pie(values=lang_dist.values, names=lang_dist.index, title="Top 10 Languages", hole=0.3)
            st.plotly_chart(fig, use_container_width=True)

        with col2:
            st.subheader("Stars vs Forks Correlation")
            fig = px.scatter(filtered_data, x='stars_count', y='forks_count', color='language',
                             size='contributors', hover_data=['repositories'], log_x=True, log_y=True,
                             title="Stars vs Forks (log scale)")
            st.plotly_chart(fig, use_container_width=True)

    with tab2:
        st.subheader("Top Repositories")
        top_repos = filtered_data.nlargest(10, 'stars_count')[['repositories', 'language', 'stars_count', 'forks_count', 'contributors']]
        ...
        fig = px.scatter(filtered_data, x='issues_count', y='contributors', color='language', size='stars_count',
                         hover_data=['repositories'], title="Issues vs Contributors Correlation")
        st.plotly_chart(fig, use_container_width=True)
    ...
    with tab3:
        if 'created_at' in filtered_data.columns:
            ...
            fig = px.line(x=monthly_trends.index, y=monthly_trends.values, title="Monthly Repository Creation Trend")
            st.plotly_chart(fig, use_container_width=True)

        st.subheader("Repository Age Distribution")
        if 'created_at' in filtered_data.columns:
            filtered_data['age'] = (datetime.now() - filtered_data['created_at']).dt.days
            fig = px.histogram(filtered_data, x='age', title="Repository Age Distribution", nbins=30)
            st.plotly_chart(fig, use_container_width=True)
    ...
    with tab4:
        ...
        if 'country' in filtered_data.columns:
            ...
            fig = px.choropleth(country_dist, locations="country", locationmode='country names',
                                color="count", hover_name="country", title="Geographical Distribution of Repositories")
            st.plotly_chart(fig, use_container_width=True)
    ...
    st.download_button(label="Download Filtered GitHub Data as CSV",
